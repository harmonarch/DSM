...
// 1.6 峰谷时段判定（官方：北京时间周一至五 9:00–12:00、14:00–18:00 为高峰，其余为低谷，周末全天低谷）
...
// 11.5 更新兜底：/releases/latest 302 Location 解析最新 tag（API 限流时走网页端，对齐 macOS 自测）
...
"""
    ...
    // 固定时间戳的样例一旦跨月就会落在窗口外被过滤，聚合断言随之失效（跨月时间炸弹）
    var appModelCal = Calendar(identifier: .gregorian)
    appModelCal.timeZone = MonthUsage.platformTimeZone
    let appModelNow = appModelCal.dateComponents([.year, .month], from: Date())
    let appModelMonthStartTs = Int(appModelCal.date(
        from: DateComponents(year: appModelNow.year!, month: appModelNow.month!, day: 1)
    )!.timeIntervalSince1970)
    let appModelSecondDayTs = appModelMonthStartTs + 86400
    let appModelAmountJSON = """
{"code":0,"msg":"","data":{"biz_code":0,"biz_msg":"","biz_data":{"start":\(appModelMonthStartTs),"end":\(appModelMonthStartTs + 86400 * 31),"bucket":86400,"models":["deepseek-v4-pro"],"series":[{"api_key":{"tracking_id":"test-tracking","name":"test-key","sensitive_id":"sk-xxx","valid":true},"model":"deepseek-v4-pro","buckets":[{"time":\(appModelMonthStartTs),"usage":{"REQUEST":2,"RESPONSE_TOKEN":100}},{"time":\(appModelSecondDayTs),"usage":{"REQUEST":5,"RESPONSE_TOKEN":200}}]}]}}}
"""
    let appModelCostJSON = """
{"code":0,"msg":"","data":{"biz_code":0,"biz_msg":"","biz_data":{"start":\(appModelMonthStartTs),"end":\(appModelMonthStartTs + 86400 * 31),"bucket":86400,"models":["deepseek-v4-pro"],"data":[{"currency":"CNY","series":[{"api_key":{"tracking_id":"test-tracking","name":"test-key","sensitive_id":"sk-xxx","valid":true},"model":"deepseek-v4-pro","buckets":[{"time":\(appModelMonthStartTs),"cost":"1.5"},{"time":\(appModelSecondDayTs),"cost":"2.5"}]}]}]}}}
"""
    func routerJSON(_ expired: Bool, amount: String, cost: String) -> ((URLRequest) throws -> (HTTPURLResponse, Data)) {
        { request in
            ...
            case "/api/v0/usage/by_api_key/amount": body = expired ? expiredJSON : amount
            case "/api/v0/usage/by_api_key/cost": body = cost
    ...
    await Task { @MainActor in
        StubURLProtocol.handler = routerJSON(false, amount: appModelAmountJSON, cost: appModelCostJSON)
        ...
        StubURLProtocol.handler = routerJSON(true, amount: appModelAmountJSON, cost: appModelCostJSON)
