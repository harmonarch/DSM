...
// MARK: - 峰谷时段判定（DeepSeek 官方峰谷定价，对齐 macOS Formatting.swift）

/// 峰谷时段（官方定价页：https://api-docs.deepseek.com/zh-cn/quick_start/pricing）
public enum TariffPeriod: Equatable {
    /// 高峰时段（单价为低谷时段的两倍）
    case peak
    /// 低谷（空闲）时段，价格为高峰的一半
    case offPeak
...
public func tariffPeriod(on date: Date, calendar: Calendar = MonthUsage.platformCalendar) -> TariffPeriod {
...
// MARK: - 版本号比较（应用内更新判断用，对齐 macOS Formatting.swift）
...
public func latestTag(fromReleaseRedirectPath path: String) -> String? {
